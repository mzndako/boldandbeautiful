...
//		$email_msg		.=	"Your account type : ".$account_type."<br />";
	...
	function password_reset_email($new_password = '' , $account_type = 'users' , $email = '')
	{
		if(c()->is_email($email)){
			c()->where(array('email' => $email));
		}else{
			c()->where(array('phone' => $email));
		}
		$query			=	$this->db->get($account_type );
		if($query->num_rows() > 0)
		{
			
			$email_msg	=	"Password Reset Successful<br />";
	...
	/***custom email sender****/
	function do_email($msg=NULL, $sub=NULL, $to=NULL, $from=NULL,$save = true)
	{
		...
		if($from == NULL)
			$from		=	$this->c_->get_setting('system_email');
		...
		$msg	=	$msg."<br /><br /><br /><br /><br /><br /><br /><hr /><center><a href=\""."#"."\">&copy; Bold & Beautiful</a></center>";
