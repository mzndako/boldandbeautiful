...
<div align="right">
	<?php echo form_open(base_url() . '?admin/view_payments/search/' , array('class' => 'form-horizontal  validate','method'=>"post"));?>
	<table cellpadding="0" cellspacing="0" border="0" class="table table-bordered mytable" style="width: 100%;" border="1">
		<tr>
			<th>Staff Records</th>
...
<?php if($show):?>
	...
	if(!s()->hAccess('view_all_payments')){
		d()->where("staff_id", s()->userdata("login_user_id"));
	}else if($staff != 0){
		d()->where("staff_id", $staff);
	}
	...
	<?php if(c()->get_view_type('income') == $view_type):
		...
		$payments = c()->get("income")->result_array();

		?>
		<h3>Showings <?=count($payments);?> Payment records from <b><?=convert_to_datetime($startdate,"g:i A (j F, Y)");?></b> to <b><?=convert_to_datetime($enddate, "g:i A (j F, Y)");?></b></h3>
					<table class="table table-bordered datatable table-striped" id="table_export">
						...
						foreach ($payments as $row):
							...
							<tr>
								...
								<td width="200px"><b><label class="label label-danger"><?php echo convert_to_date($row['date'],"g:i A");?></label> <label class="label label-warning"><?php echo convert_to_date($row['date']);?></label> </b></td>
								<?php if($staff == 0):?>
								...
								<td><?php echo @$services[$row['product']];?></td>
								<td>
									N <?php echo "<span class='format_number'>".$row['amount']."</span> x ".$row['unit'];?>
								</td>
								<td><?php
									if($row['discount'] > 0)
										echo "<label class='label label-info'>".$row['discount']." % </label>";
									else
										echo "No Discount";

									?></td>
								<td >
									<b>N <span class="format_number"><?php $total += $row['total']; echo $row['total'];?></span></b>
								</td>
	...
	<h4 style=" color: red;">TOTAL: <b style=" margin-right: 5px;">N <span class="format_number"><?=$total;?></span></b></h4>
</span>


	<?php elseif(c()->get_view_type("expenditure") == $view_type):

		$expenses = c()->get("expenditure")->result_array();
		?>

		<h3>Showings <?=count($expenses);?> Payment records from <b><?=convert_to_datetime($startdate,"g:i A (j F, Y)");?></b> to <b><?=convert_to_datetime($enddate, "g:i A (j F, Y)");?></b></h3>
		<table class="table table-bordered datatable table-striped" id="table_export">
			<thead>
			<tr>
				<th>#</th>
...
<?php elseif(c()->get_view_type("income & expenditure") == $view_type):
		...
		d()->where("date >=",database_date($startdate));
		d()->where("date <=",database_date($enddate));
		if(!s()->hAccess('view_all_payments')){	d()->where("staff_id", s()->userdata("login_user_id"));
		}else if($staff != 0){ d()->where("staff_id", $staff);}
		$expenses = c()->get("expenditure")->result_array();
		...
		?>
		<h3>INCOME AND EXPENDITURE SUMMARY REPORT: <b><?=convert_to_datetime($startdate,"g:i A (j F, Y)");?></b> - <b><?=convert_to_datetime($enddate, "g:i A (j F, Y)");?></b></h3>
		<table class="table table-bordered datatable table-striped" >
			...
			<tbody>
			<tr>
				<td><b>N <b  class="format_number"><?=$total_income;?></b></b></td>
				<td><b>N <b  class="format_number"><?=$total_expenses;?></b></b></td>
				<td><b>N <b  class="format_number"><?=$balance;?></b></b></td>
			</tr>
			</tbody>
		</table>
